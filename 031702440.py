...
class PeopleInfo:
    ...
    def cut_addr(self, ad):
        ...
        if (ret):
            ...
            return self.__addr
        ret = re.search("([^0-9]+?省|.+?自治区)(.{1,4}?市|.{4,8}?自治州|.{2,4}?地区|.{2,4}?盟)((.{1,4}?区|[^0-9]{1,3}?县|.{1,3}?市|.{2,6}?旗)?)((.{1,4}?街道|.{1,4}?镇|.{1,4}?乡|.+?开发区|.{2,3}?管委会)?)((.{1,6}?弄|.{1,6}?路|.{1,6}?街|.{1,6}?巷|.{1,6}?道)?)((\d+?号)?)(.*)",
            ad)
        if (ret):
            self.__addr.append(ret.group(1))
            ...
            self.__addr.append(ret.group(11).rstrip('.'))
            return self.__addr
        ret = re.search("("+province+ "?)" + "((省|自治区)?)" + "(" + city1 + city2 + city3 + city4 + city5 + city6 + city7 + city8 + city9 + "?)" + r"([市]?)((.{0,4}?区|[^0-9]{0,3}?县|.{0,3}?市)?)((.{0,4}?街道|.{0,4}?镇|.{0,4}?乡|.+?开发区|.{2,3}?管委会)?)((.{0,6}?路|.{0,6}?街|.{0,6}?巷|.{0,6}?道)?)((\d+?号)?)(.*)",
            ad)
        if (ret):
            if (ret.group(1) != ''):
